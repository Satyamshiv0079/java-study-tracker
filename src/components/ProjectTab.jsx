  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm transition hover:border-slate-700">
        <h2 className="text-xl font-bold text-white mb-2">Capstone Project Tracker</h2>
        <p className="text-sm text-slate-400 mb-6">A real-world project is crucial. Build a Spring Boot REST API for a task tracker, paired with a React frontend.</p>
          ...
          {projectMilestones.map((m) => (
            ...
            >
              <div className={`mt-0.5 h-5 w-5 rounded flex items-center justify-center border flex-shrink-0 transition-colors ${
                m.done ? 'bg-emerald-500 border-emerald-500 text-slate-950 shadow-[0_0_8px_rgba(16,185,129,0.4)]' : 'bg-slate-900 border-slate-700'
              }`}>
              ...
              <div>
                <p className={`text-sm font-medium ${m.done ? 'text-emerald-400 line-through opacity-70' : 'text-slate-200'}`}>{m.name}</p>
              </div>
            </button>
          ))}
        </div>
