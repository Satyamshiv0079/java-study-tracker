...
export default function SyllabusTab({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 h-[calc(100vh-14rem)] overflow-y-auto flex flex-col gap-2.5 custom-scrollbar">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2 mb-2">45-Day Study Log</h3>
          ...
          {Array.from({ length: 45 }, (_, i) => i + 1).map((dayNum) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-2 rounded-lg border transition-all text-xs flex items-center justify-between gap-2 hover:border-slate-700 ${
                  activeDay === dayNum
                    ? 'bg-indigo-600/20 border-indigo-500 text-white font-semibold shadow-[0_0_8px_rgba(79,70,229,0.3)]'
              ...
              >
                <div className="flex flex-col truncate">
                  ...
                  <span className="truncate text-slate-200 mt-0.5">{item.title}</span>
                </div>
                {isCompleted ? (
                  ...
                  <span className="h-4 w-4 rounded-full border border-slate-700 flex-shrink-0"></span>
                )}
              </button>
            );
          })}
      ...
      <div className="lg:col-span-3 space-y-6">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm transition hover:border-slate-700">
          ...
          <h2 className="text-2xl font-bold text-white">Day {activeDay}: {dayData.title}</h2>
          {dayData.isTemplate && (
          ...
          <h3 className="text-sm font-bold text-slate-300 mt-5 mb-2.5">Learning objectives:</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {dayData.topics.map((t, idx) => (
              <div key={idx} className="bg-slate-950 border border-slate-800/80 rounded-lg p-3 flex items-start gap-2.5 transition hover:border-slate-700">
                ...
                <p className="text-xs text-slate-300 font-medium">{t}</p>
              </div>
            ))}
          </div>

          <h3 className="text-sm font-bold text-slate-300 mt-6 mb-2">Theory:</h3>
          <div className="bg-slate-950 border border-slate-800/80 rounded-lg p-4 font-mono text-xs leading-relaxed text-slate-300 whitespace-pre-wrap">
            {dayData.theory}
            {activeDay === 1 && (
              <div className="bg-slate-900 p-3 rounded-lg border border-slate-800 mt-3">
          ...
          {dayData.youtubeId && (
            <>
              <h3 className="text-sm font-bold text-slate-300 mt-6 mb-2 flex items-center gap-2">
                <svg className="w-4 h-4 text-red-500" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.5 12 3.5 12 3.5s-7.505 0-9.377.55a3.016 3.016 0 0 0-2.122 2.136C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.55 9.376.55 9.376.55s7.505 0 9.377-.55a3.016 3.016 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
                </svg>
                Video Reference:
              </h3>
              <div className="bg-slate-950 border border-slate-800/80 rounded-lg p-2 overflow-hidden aspect-video relative group">
                <iframe 
          ...
          <div className="mt-6 pt-5 border-t border-slate-800 flex justify-between gap-4 items-center">
            <div className="text-xs text-slate-400">Need this explained properly? Ask the real AI Mentor.</div>
            <button
