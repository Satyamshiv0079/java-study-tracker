...
const StatCard = ({ label, value, pct, color, sub, highlight }) => (
  <div className={`bg-slate-900 border rounded-xl p-5 relative overflow-hidden ${highlight ? 'border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]' : 'border-slate-800'}`}>
    <div className={`absolute top-0 left-0 w-full h-1 bg-${color}-500/20`}>
      <div className={`h-full bg-${color}-500`} style={{ width: `${pct}%` }}></div>
    </div>
    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1 mt-1">{label}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 relative transition-all hover:border-slate-700 hover:shadow-lg hover:shadow-indigo-900/10">
            <div className="absolute top-4 right-4 flex gap-2">
              <button onClick={() => handleToggleDayComplete(activeDay)} className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1.5 transition ${completedDays.includes(activeDay) ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.2)]' : 'bg-slate-800 text-slate-400 border border-slate-700 hover:bg-slate-700'}`}>
            ...
            <h2 className="text-2xl font-bold mt-3 text-white flex items-center gap-2">
              <span className="text-indigo-400 font-mono">Day {activeDay}:</span> {dayData.title}
            </h2>

            <p className="text-sm text-slate-400 mt-2 line-clamp-3 leading-relaxed">{dayData.theory}</p>
            ...
            <div className="mt-5 flex flex-wrap gap-2.5 mb-4">
              ...
              <button onClick={() => setCurrentTab('coding')} className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 hover:border-slate-600 border border-transparent text-slate-200 text-xs font-bold transition-all">
                Open DSA Practice
              </button>
            </div>
            
            {dayData.youtubeId && (
              <div className="mt-4 bg-slate-950 border border-slate-800/80 rounded-lg overflow-hidden aspect-video relative group">
                <iframe 
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm">
            <h3 className="text-base font-bold text-white mb-4">Pick a day</h3>
            ...
            <p className="text-[10px] text-slate-500 mt-3">Red dot = no hand-written lesson for that day yet (ask the AI Mentor to fill the gap, or add your own notes).</p>
          </div>
        </div>

        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 transition hover:border-slate-700">
            <h3 className="text-sm font-bold text-white mb-3">Day {activeDay} Notebook</h3>
            <textarea
              value={activeNote}
              onChange={(e) => setActiveNote(e.target.value)}
              placeholder="Write self-notes, algorithms or cheat-sheet notes here..."
              className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 focus:shadow-[0_0_8px_rgba(79,70,229,0.2)] rounded-lg p-2.5 text-xs text-slate-300 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono resize-none h-24 transition"
            />
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 text-xs text-slate-400 space-y-2">
            <p className="font-bold text-slate-200 text-sm mb-1">What's real here vs. self-reported</p>
            <p>Days/DSA/hours are things <span className="text-slate-300">you</span> mark done - nothing verifies you actually did the work. The readiness % is a rough weighted heuristic, not a real assessment. The AI Mentor and code review genuinely call Claude. Treat this as a tracker + study aid, not a certification.</p>
          </div>
        </div>
      </div>
    </div>
